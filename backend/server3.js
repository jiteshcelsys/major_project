...
app.post("/PreviousFormClick", async (req, res) => {
  ...
  const backend =await pool.query('select * from emp_login where emp_num=$1 order by indi_obj',[req.body.emp_num])
...
app.post("/emp_login", async (req, res) => {
  console.log(req.body);
  const sqlData= await pool.query("select * from emp_login where emp_num=$1 and indi_obj=$2  " ,[req.body.emp_num,req.body.indi_obj] )
//  const managerData=('Select * from managertable')
  if(sqlData.rows.length===0){
    const InserEmpLogin = await pool.query(
      "Insert into emp_login (emp_num,indi_obj,perfor_obj, self_rating)values( $1,$2,$3,$4)",
      [
        req.body.emp_num,
        req.body.indi_obj,
        req.body.perfor_obj,
...
app.get("/manager", async (req, res) => {
  ...
  const manager = await pool.query(
    "select e.emp_num, u.first_name from emp_login e inner join userdetails u on e.emp_num=u.emp_num group by (e.emp_num, u.first_name) order by emp_num"
  );
